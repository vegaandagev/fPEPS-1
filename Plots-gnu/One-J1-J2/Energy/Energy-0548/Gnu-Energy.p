...
G(x)=a+b*(x**(2))+c*(x**(4))
fit [0.10:0.34]  G(x) "ipeps.txt" u ($1):($3)    via a,b,c
#M(x)=a1+b1*(x)
...
#fit  [0.10:0.34] P(x) "ipeps.txt" u ($1):($3)    via a1,b1,c1
...
p [0.0:0.40]    x<0.330 ? G(x) : 1/0 notitle lw 4 lc rgb "#1E90FF" dt 4,   "ipeps.txt" u ($1):($2)  t 'Stripe' with points  pointtype 2 lw 3  ps 8.0 lc rgb "#4B0082", "ipeps.txt" u ($1):($3)  t 'Neel' with points  pointtype 4 lw 3  ps 8.0 lc rgb "#FF1493", "ipeps.txt" u ($1):($5)  t 'VBS' with points  pointtype 8 lw 3  ps 8.0 lc rgb "#FFA500","ipeps.txt" u ($1):($4)  t 'Haldane' with points  pointtype 6 lw 3  ps 8.0 lc rgb "#008080"
