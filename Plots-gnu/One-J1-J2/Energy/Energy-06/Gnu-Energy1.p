...
G(x)=a2+b2*(x)
fit [0.10:0.33]  G(x)  "ipeps.txt" u ($1):($6)    via a2,b2#,c
...
set output "Mag.eps"
p [0.0:0.40]   "ipeps.txt" u ($1):($3) t "m" with points  pointtype 2 lw 3  ps 8.0 lc rgb "#8B008B","ipeps.txt" u ($1):($4) t "{/Symbol D}T_{x}" with points  pointtype 4 lw 3  ps 8.0 lc rgb "#C71585","ipeps.txt" u ($1):($6) t "{/Symbol D}T_{x-y}" with points  pointtype 6 lw 3  ps 8.0 lc rgb "#FF4500", x<0.330 ? M(x) : 1/0 notitle lw 4 lc rgb "#DC143C" dt 4, x<0.330 ? G(x) : 1/0 notitle lw 4 lc rgb "#DC143C" dt 4
