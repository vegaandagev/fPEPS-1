...
#fit  P(x) "EKagome.txt"  u ($1):($3)  via v1, z1#,c1 
#fit  M(x) "EKagome.txt"  u ($1):($3)  via a1#, b1#,c1 
...
set output "Mag.eps"
p [0.0:0.50]  [0:0.3]  "EKagome.txt"  u ($1):($3)  t "J_{{/Symbol c}}=0"  with points  pointtype 2 lw 3  ps 8.0 lc rgb "#8B008B","Echiral5.txt"  u ($1):($3)  t "J_{{/Symbol c}}=0.5"  with points  pointtype 4 lw 3  ps 8.0 lc rgb "#ce5c00", M(x) notitle with line dt 4 lw 4 linetype 4 lc rgb "#8A2BE2", G(x) notitle with line dt 4 lw 4 linetype 4 lc rgb "#8A2BE2"

#, P(x) notitle with line dt 3 lw 3 linetype 3 lc rgb "#ce5c00"
