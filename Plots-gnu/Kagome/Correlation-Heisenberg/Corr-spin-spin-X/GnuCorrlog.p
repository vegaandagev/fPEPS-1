...
M(x)=d1*x**c1
fit [4:12]  M(x) "CorrelationH-one.txt" u ($1):(abs($2))   via c1,d1 
...
set logscale x 10
set logscale y 10
set ytics 1e-12, 10, 1
#set xtics 1e-12, 10, 1
set mytics 10
set mxtics 10
set ytics add ("10^{-1}" 0.1,"10^{-2}" 0.01, "10^{-3}" 0.001, "10^{-4}" 0.0001,"10^{-5}" 0.00001, "10^{-6}" 0.000001, "10^{-7}" 0.0000001,"10^{-8}" 0.00000001,"10^{-9}" 0.000000001,"10^{-10}" 0.0000000001,"10^{-11}" 0.00000000001,"10^{-11}" 0.00000000001,"10^{-12}" 0.000000000001)
...
p  [1:30]   "CorrelationHD6-two.txt"  u ($1):(abs($2)) t 'two-layer:D=6'  with points  pointtype 9 lw 3  ps 8 lc rgb "#DC143C","CorrelationHD6-one.txt"  u ($1):(abs($2)) t 'one-layer:D=6'  with points  pointtype 8 lw 3  ps 8 lc rgb "#008080","CorrelationH-two.txt"  u ($1):(abs($2)) t 'two-layer:D=5'  with points  pointtype 7 lw 3  ps 8 lc rgb "#9400D3","CorrelationH-one.txt"  u ($1):(abs($2)) t 'one-layer:D=5'  with points  pointtype 6 lw 3  ps 8 lc rgb "#FF1493"
