...
print "Norm_val_boundry", Env_left[0].product(Env_right[1])
#for i in xrange(N_x):
...
##############QR-Left#####################################################
   for q in xrange(0,N_x-1):
    ...
    #print   Norm_val_boundry,  MPS_R.norm(),  (Norm_val_boundry-MPS_R.norm())/Norm_val_boundry
...
################Right-Move######################
   for q in reversed(xrange(1,N_x)):
    ...
    PEPS_listten[q], PEPS_listten[q-1], E_coulmn[q], E_row[q-1]=UD.Update_E_H( PEPS_listten[q], PEPS_listten[q-1], U_ham, U_ham0, U_hamN, q, N_x, D, d, N_y, H, H0, HN, chi_express, cond_list)
    #print E_coulmn[q]
    ...
    #print  "norm_test", Norm_val_boundry,  MPS_R.norm(),  (Norm_val_boundry-MPS_R.norm())/Norm_val_boundry
    #N_iter_list.append((Norm_val_boundry-MPS_R.norm())/Norm_val_boundry)
...
#   #E_0=(count1+count)/(N_x*N_y)
   ...
   print "E_total", count, count1, E_1, len(E_iter_list)
...
file = open("Cond.txt", "w")
for index in range(len(cond_list)):
  file.write(str(index) + " " + str(cond_list[index])+" "+ "\n")
file.close()
